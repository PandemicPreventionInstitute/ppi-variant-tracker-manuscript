...
data {
  ...
  int<lower=1> N_future; // number of days that have been observed by 7-01
  ...
  int fut_trials[K, N_future];  // total number of trials (seqs) in the country x day in the most recent dataset
}
...
model {
  ...
  mu_hier_raw ~ normal(-1, .5); // scale mu_hier_scaled to be on unit scale
...
generated quantities {
  ...
  int Y_tilde[K, N+num_days_nowcast, ncat];
  ...
  for(k in 1:K){
    ...
    for(i in 1:(N+num_days_nowcast)){
      ...
      // Generate predicted count with future number of sequences
      if(i <= N_future){
        
        if(fut_trials[k, i] > 0){ 
            
            Y_tilde[k, i] = multinomial_rng(softmax(eta), fut_trials[k, i]);
