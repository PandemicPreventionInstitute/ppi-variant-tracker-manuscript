...
for (lineage_to_show in c('BA.2', 'BA.4', 'BA.5', 'BA.2.12.1', 'BA.1', 'other')) {
 
    
    p1 <- d %>%
    filter(lineage == 'BA.5') %>%
    ggplot() +
    geom_point(aes(collection_date,
                   as.double(p_lineage),
...
FROM read_parquet('", contrast, "', filename  = TRUE)
    ...
    geom_line()+
    geom_ribbon(aes(x = reference_date,
                ymin = estimate - 2 * SE,
                ymax = estimate + 2 * SE),
                alpha = 0.2) +
    facet_wrap(~contrast)+
...
SELECT
    s.country,
    s.reference_date,
    TRY_CAST(s.spline_bs AS FLOAT) AS spline_bs,
    TRY_CAST(m.BS_mean_mc AS FLOAT) AS mc_bs
FROM
...
WHERE m.country in ('Portugal', 'Brazil', 'Spain', 'India', 'Czechia')
                  "))  %>%
    pivot_longer(c("spline_bs", "mc_bs"))

p3 <- ggplot(d, aes(x = reference_date,
               y = value,
               group = name,
               color = name)) +
    geom_point() +
